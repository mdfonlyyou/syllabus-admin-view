...
<script>
  import {getLessonAssessPage} from '../../assets/lesson'
  import {deleteLessonAssessByLidAndAid} from '../../assets/lesson'
  ...
  export default {
    ...
    methods: {
      ...
      //跳转到修改页面
      edit(row){
        this.$router.push({path: '/edit_lesson_assess', query: {lid: row.fk_to_id, aid: row.id}});
      },
      //列出评论列表
      assessPageInfos(){
        var vm = this;
        ...
        vm.loading=true;
        getLessonAssessPage(vm.$route.query.lid, para).then(response => {
      ...
      handleDel(row){
        ...
        this.$confirm('确认删除该评价吗?', '提示', {
          ...
          vm.loading=true;
          deleteLessonAssessByLidAndAid(row.fk_to_id, row.id).then(response => {
            vm.$message({
              message: '删除评价成功',
              type: 'success'
            });
