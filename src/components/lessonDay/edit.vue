<template>
  <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="80px" class="demo-ruleForm"
    ...
    <el-form-item label="简介" prop="guide" style="padding-top: 20px">
      ...
      <quill-editor v-model="ruleForm2.guide" ref="myQuillEditor" :options="editorOption" @blur="onEditorBlur($event)"
                    @focus="onEditorFocus($event)"
      >
      </quill-editor>

      <!-- 文件上传input 将它隐藏-->
      <el-upload class="upload-demo" action="http://avatarcn.oss-cn-hangzhou.aliyuncs.com" :before-upload='beforeUpload'
                 :data="data3" :on-success='upScuccess'
...
<script>
  import {getLessonDay} from '../../assets/lesson'
  import {editLessonDay} from '../../assets/lesson'
  ...
  export default {
    ...
    methods: {
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            editLessonDay(_this.$route.query.lid,_this.$route.query.did,para).then(response => {
      ...
      getLessonDayInfo(){
        ...
        getLessonDay(this.$route.query.lid, this.$route.query.did).then(response => {
          this.ruleForm2.days=response.data.days;
          this.ruleForm2.guide=response.data.guide;
